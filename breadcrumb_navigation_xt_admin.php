...
Description: Adds a breadcrumb navigation showing the visitor&#39;s path to their current location. For details on how to use this plugin visit <a href="http://mtekk.weblogs.us/code/breadcrumb-nav-xt/">Breadcrumb NavXT</a>. 
...
include(dirname(__FILE__)."/breadcrumb_navigation_xt_class.php");
//Include the language file
if(get_option('bcn_language'))
{
	include(dirname(__FILE__)."/languages/" . get_option('bcn_language') . ".php");
}
else
{
	include(dirname(__FILE__)."/languages/english.php");
}
//Security function
...
function bcn_language_set()
{
	global $wpdb, $bcn_admin_req;
	bcn_security();
	update_option('bcn_language', bcn_get('language'));
	include(dirname(__FILE__)."/languages/" . get_option('bcn_language') . ".php");
	//Update textual options per the language file
	update_option('bcn_title_home', TXT_BCN_TITLE_HOME);
	update_option('bcn_title_blog', TXT_BCN_TITLE_BLOG);
	update_option('bcn_search_prefix', TXT_BCN_SEARCH_PREFIX);
	update_option('bcn_search_suffix', TXT_BCN_SEARCH_SUFFIX);
	update_option('bcn_author_prefix', TXT_BCN_AUTHOR_PREFIX);
	update_option('bcn_author_suffix', TXT_BCN_AUTHOR_SUFFIX);
	update_option('bcn_singleblogpost_prefix', TXT_BCN_SINGLEBLOGPOST_PREFIX);
	update_option('bcn_singleblogpost_suffix', TXT_BCN_SINGLEBLOGPOST_SUFFIX);
	update_option('bcn_page_prefix', TXT_BCN_PAGE_PREFIX);
	update_option('bcn_page_suffix', TXT_BCN_PAGE_SUFFIX);
	update_option('bcn_urltitle_prefix', TXT_BCN_URLTITLE_PREFIX);
	update_option('bcn_urltitle_suffix', TXT_BCN_URLTITLE_SUFFIX);
	update_option('bcn_archive_category_prefix', TXT_BCN_ARCHIVE_CATEGORY_PREFIX);
	update_option('bcn_archive_category_suffix', TXT_BCN_ARCHIVE_CATEGORY_SUFFIX);
	update_option('bcn_archive_date_prefix', TXT_BCN_ARCHIVE_DATE_PREFIX);
	update_option('bcn_archive_date_suffix', TXT_BCN_ARCHIVE_DATE_SUFFIX);
	update_option('bcn_archive_date_format', 'EU');
	update_option('bcn_attachment_prefix', TXT_BCN_ATTACHMENT_PREFIX);
	update_option('bcn_attachment_suffix', TXT_BCN_ATTACHMENT_SUFFIX);
	update_option('bcn_archive_tag_prefix', TXT_BCN_ARCHIVE_TAG_PREFIX);
	update_option('bcn_archive_tag_suffix', TXT_BCN_ARCHIVE_TAG_SUFFIX);
	update_option('bcn_title_404', TXT_BCN_TITLE_404);
	update_option('bcn_current_item_urltitle', TXT_BCN_CURRENT_ITEM_URLTITLE);
	update_option('bcn_current_item_style_prefix', TXT_BCN_CURRENT_ITEM_STYLE_PREFIX);
	update_option('bcn_current_item_style_suffix', TXT_BCN_CURRENT_ITEM_STYLE_SUFFIX);
	update_option('bcn_posttitle_maxlen', 0);
	update_option('bcn_paged_prefix', TXT_BCN_PAGED_PREFIX);
	update_option('bcn_paged_suffix', TXT_BCN_PAGED_SUFFIX);
	update_option('bcn_singleblogpost_category_prefix', TXT_BCN_SINGLEBLOGPOST_CATEGORY_PREFIX);
	update_option('bcn_singleblogpost_category_suffix', TXT_BCN_SINGLEBLOGPOST_CATEGORY_SUFFIX);
}
...
function bcn_admin()
{
	...
	{ ?>
		<div id="message" class="updated fade">
			<p><?php echo TXT_BCN_VERSION_WARNING; ?></p>
			<p><?php echo TXT_BCN_VERSION_CORE . $bcn_version; ?>.</p>
			<p><?php echo TXT_BCN_VERSION_ADMIN . $bcn_admin_version; ?>.</p>
		</div>
	<?php }
	//Check to see if the language has been set yet, if not force the user to do that now.
	if(get_option('bcn_language') == NULL)
	{
	?>
		<div class="wrap"><h2>Breadcrumb NavXT Initial Configuration</h2>
		<p><strong>Welcome. Willkommen. Bienvenidos. Tervetuloa. Huan yin. Bienvenue. Velkomen. Irashaimasu. Benvenuto. </strong><br />
		Thankyou for choosing Breadcrumb NavXT as your WordPress breadcrumb generation plug-in. Please 
		select the language that you want to view the administration interface in, as well as setup the
		default values for.</p>
		<form action="options-general.php?page=breadcrumb-nav-xt" method="post" id="bcn_xt_admin_options">
			<fieldset id="language">
				<legend>Select You Language</legend>
				<p>
						<?php 
							//Populate the array
							$bcn_location = dirname(__FILE__) . "/languages/";
							$bcn_opta = bcn_files($bcn_location);
							sort($bcn_opta);
						?>
					<select name="language" style="font-size: 1.5em;">
						<?php foreach($bcn_opta as $option)
						{
							if($option != get_option('bcn_language'))
							{
								echo "<option>" . basename($option, ".php") . "</option>";
							}
						}?>
					</select>
					<input type="submit" name="bcn_language_set" style="font-size: 1.5em;" value="Save &raquo;" />
				</p>
			</fieldset>
		</form>
		</div>
	...
	<form action="options-general.php?page=breadcrumb-nav-xt" method="post" id="bcn_admin_options">
		<fieldset id="general">
			...
			<p>
				...
				<?php echo TXT_BCN_KEEP_SPACES; ?></label>
			</p>
			<p> 
					<?php 
						echo TXT_BCN_LANGUAGE;
						//Populate the array
						$bcn_location = dirname(__FILE__) . "/languages/";
						$bcn_opta = bcn_files($bcn_location);
						sort($bcn_opta);
				...
				<select name="language">
					<option><?php echo get_option('bcn_language'); ?></option>
					<?php foreach($bcn_opta as $option)
					{
						if(basename($option, ".php") != get_option('bcn_language'))
						{
							echo "<option>" . basename($option, ".php") . "</option>";
						}
					}?>
		...
		<fieldset id="static_front_page">
			...
			<p><?php echo TXT_BCN_DISPLAY_HOME_LINK; ?>
		...
		<fieldset id="category">
			<legend><?php echo TXT_BCN_ARCHIVE_DISPLAY_SETTINGS; ?></legend>
			<p>
				<label for="urltitle_prefix"><?php echo TXT_BCN_URL_TITLE_PREFIX; ?></label>
				<input type="text" name="urltitle_prefix" id="urltitle_prefix" value="<?php echo get_option('bcn_urltitle_prefix'); ?>" size="32" />
			</p>
			<p>
				<label for="urltitle_suffix"><?php echo TXT_BCN_URL_TITLE_SUFFIX; ?></label>
				<input type="text" name="urltitle_suffix" id="urltitle_suffix" value="<?php echo get_option('bcn_urltitle_suffix'); ?>" size="32" />
			</p>
			<p>
				<label for="archive_category_prefix"><?php echo TXT_BCN_ARCHIVE_CATEGORY_PREFIX_TEXT; ?></label>
			...
			<p>
				<label for="archive_category_suffix"><?php echo TXT_BCN_ARCHIVE_CATEGORY_SUFFIX_TEXT; ?></label>
			...
			<p>
				<label for="archive_date_prefix"><?php echo TXT_BCN_ARCHIVE_DATE_PREFIX_TEXT; ?></label>
			...
			<p>
				<label for="archive_date_suffix"><?php echo TXT_BCN_ARCHIVE_DATE_SUFFIX_TEXT; ?></label>
			...
			<p><?php echo TXT_BCN_ARCHIVE_DATE_FORMAT; ?> 
			...
			<p>
				<label for="archive_tag_prefix"><?php echo TXT_BCN_ARCHIVE_TAG_PREFIX_TITLE; ?></label>
				<input type="text" name="archive_tag_prefix" id="archive_tag_prefix" value="<?php echo get_option('bcn_archive_tag_prefix'); ?>" size="32" />
			</p>
			<p>
				<label for="archive_tag_suffix"><?php echo TXT_BCN_ARCHIVE_TAG_SUFFIX_TITLE; ?></label>
				<input type="text" name="archive_tag_suffix" id="archive_tag_suffix" value="<?php echo get_option('bcn_archive_tag_suffix'); ?>" size="32" />
			</p>
		</fieldset>
		<fieldset id="current">
			<legend><?php echo TXT_BCN_CURRENT_ITEM_SETTINGS; ?></legend>
			<p><?php echo TXT_BCN_LINK_CURRENT_ITEM; ?> 
			...
			<p>
				<label for="current_item_urltitle"><?php echo TXT_BCN_CURRENT_ITEM_URL_TITLE; ?></label>
			...
			<p>
				<label for="current_item_style_prefix"><?php echo TXT_BCN_CURRENT_STYLE_PREFIX_TEXT; ?></label>
			...
			<p>
				<label for="current_item_style_suffix"><?php echo TXT_BCN_CURRENT_STYLE_SUFFIX_TEXT; ?></label>
			...
			<p><?php echo TXT_BCN_DISPLAY_PAGED_TEXT; ?> 
			...
			<p>
				<label for="paged_prefix"><?php echo TXT_BCN_PAGED_PREFIX_TEXT; ?></label>
				<input type="text" name="paged_prefix" id="paged_prefix" value="<?php echo get_option('bcn_paged_prefix'); ?>" size="32" />
			</p>
			<p>
				<label for="paged_suffix"><?php echo TXT_BCN_PAGED_SUFFIX_TEXT; ?></label>
				<input type="text" name="paged_suffix" id="paged_suffix" value="<?php echo get_option('bcn_paged_suffix'); ?>" size="32" />
			</p>
		</fieldset>
		<fieldset id="single">
			<legend><?php echo TXT_BCN_SINGLE_POST_SETTINGS; ?></legend>
			<p>
				<label for="singleblogpost_prefix"><?php echo TXT_BCN_SINGLE_POST_PREFIX; ?></label>
			...
			<p>
				<label for="singleblogpost_suffix"><?php echo TXT_BCN_SINGLE_POST_SUFFIX; ?></label>
			...
			<p>
				<label for="singleblogpost_category_prefix"><?php echo TXT_BCN_SINGLE_CATEGORY_PREFIX; ?></label>
			...
			<p>
				<label for="singleblogpost_category_suffix"><?php echo TXT_BCN_SINGLE_CATEGORY_SUFFIX; ?></label>
		...
		<input type="submit" name="bcn_admin_options" value="Save &raquo;" />
	</form>
	</div>
	<?php
	}
